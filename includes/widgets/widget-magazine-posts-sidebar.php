 ...
 */
class Poseidon_Pro_Magazine_Posts_Sidebar_Widget extends WP_Widget {
	...
	function __construct() {
		...
		// Setup Widget.
		parent::__construct(
			'poseidon-magazine-posts-sidebar', // ID.
			esc_html__( 'Magazine (Sidebar)', 'poseidon-pro' ), // Name.
			array(
				'classname' => 'poseidon-magazine-sidebar-widget',
				'description' => esc_html__( 'Displays your posts from a selected category. You can use this widget in the Main Sidebar widget area.', 'poseidon-pro' ),
	...
	function render( $settings ) {
		...
		if ( $posts_query->have_posts() ) :
			...
			set_query_var( 'poseidon_post_excerpt', (bool) $settings['excerpt'] );
	...
	function widget_title( $args, $settings ) {
		...
		if ( ! empty( $widget_title ) ) :
			...
			if ( $settings['category'] > 0 ) :
				...
				$link_title = sprintf( esc_attr__( 'View all posts from category %s', 'poseidon-pro' ), get_cat_name( $settings['category'] ) );
	...
	function form( $instance ) {
		...
		<p>
			<label for="<?php echo $this->get_field_id( 'excerpt' ); ?>">
				...
				<?php esc_html_e( 'Display post excerpt', 'poseidon-pro' ); ?>
			</label>
		</p>
