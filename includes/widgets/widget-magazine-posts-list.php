...
class Poseidon_Pro_Magazine_Posts_List_Widget extends WP_Widget {
	...
	function __construct() {
		...
		// Setup Widget
		parent::__construct(
			'poseidon-magazine-posts-list', // ID
			sprintf( esc_html__( 'Magazine Posts: List (%s)', 'poseidon-pro' ), 'Poseidon Pro' ), // Name
			array( 
				'classname' => 'poseidon_magazine_posts_list', 
				'description' => esc_html__( 'Displays your posts from a selected category in a simple list layout. Please use this widget ONLY in the Magazine Homepage widget area.', 'poseidon-pro' ),
	...
	function widget( $args, $instance ) {
		...
		if ( ! $this->is_preview() ) {
			$cache = wp_cache_get( 'widget_poseidon_magazine_posts_list', 'widget' );
		}
		if ( ! is_array( $cache ) ) {
			$cache = array();
		}
	...
	<?php
		...
		if ( ! $this->is_preview() ) {
			...
			wp_cache_set( 'widget_poseidon_magazine_posts_list', $cache, 'widget' );
		} else {
			ob_end_flush();
		}
...
?>
		...
		<p>
			<label for="<?php echo $this->get_field_id('number'); ?>"><?php esc_html_e( 'Number of posts:', 'poseidon-pro' ); ?>
...
<?php
	...
	public function delete_widget_cache() {
		...
		wp_cache_delete( 'widget_poseidon_magazine_posts_list', 'widget' );
